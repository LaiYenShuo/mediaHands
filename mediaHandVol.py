...
def Draw_hand_points(image, normalized_hand_landmarks):
  cv2.circle(
      image, normalized_hand_landmarks[4], 12, (255, 0, 255), -1, cv2.LINE_AA)
  cv2.circle(
      image, normalized_hand_landmarks[8], 12, (255, 0, 255), -1, cv2.LINE_AA)
  cv2.line(image, normalized_hand_landmarks[4],
           normalized_hand_landmarks[8], (255, 0, 255), 3)
  ...
  if results.multi_hand_landmarks:
    for hand_landmarks in results.multi_hand_landmarks:
      ...
      try:
          ...
          cv2.rectangle(image, (50, int(350-length)),
                        (85, 350), (255, 0, 0), cv2.FILLED)
          percent = int(length / 200.0 * 100)
          #print(percent)
          if percent > 100:
              percent = 100
          strRate = str(percent) + '%'
          cv2.putText(image, strRate, (40, 410),
